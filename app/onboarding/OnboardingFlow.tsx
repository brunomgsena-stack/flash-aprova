          ...
          <div className="flex flex-col items-center gap-1">
            <div
              ...
              style={{
                ...
                color: s.n <= current ? '#fff' : 'rgba(255,255,255,0.25)',
                boxShadow: s.n === current ? `0 0 16px ${VIOLET}80` : 'none',
              }}
            >
              {s.n < current ? '✓' : s.n}
            </div>
            <span className="text-xs hidden sm:block"
              style={{ color: s.n === current ? '#a78bfa' : 'rgba(255,255,255,0.20)' }}>
              {s.label}
            </span>
          </div>
          {i < steps.length - 1 && (
            <div className="w-12 sm:w-20 h-px mx-1 mb-4 sm:mb-0 transition-all duration-500"
              style={{ background: s.n < current ? `linear-gradient(90deg,${VIOLET},${CYAN})` : 'rgba(255,255,255,0.08)' }} />
          )}
        </div>
      ))}
    </div>
...
export default function OnboardingFlow() {
  ...
  async function handleSubmitLead(e: React.FormEvent) {
    ...
    // Persist results for checkout page
    ...
    >
      ...
      <div className="relative max-w-2xl mx-auto" style={{ zIndex: 1 }}>
        ...
        {step === 1 && (
          <div className="relative rounded-3xl p-7 sm:p-10" style={cardStyle}>
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <p className="text-white font-black text-xl leading-tight">Qual é o seu objetivo?</p>
                <p className="text-slate-500 text-sm mt-0.5">A IA vai calibrar seu diagnóstico com base na sua meta</p>
              </div>
            </div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-xl px-4 py-3.5 text-sm text-white outline-none transition-all placeholder-slate-700"
                style={inputStyle}
                autoComplete="off"
              />
              {showSugg && filteredSugg.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 rounded-xl overflow-hidden z-20"
                  ...
                  {filteredSugg.map(u => (
                    <button key={u} className="w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:text-white transition-colors"
            ...
            <button
              onClick={() => { if (university.trim()) setStep(2); }}
              disabled={!university.trim()}
              className="w-full py-4 rounded-xl font-black text-black text-base transition-all duration-200 hover:-translate-y-0.5 disabled:opacity-40 disabled:cursor-not-allowed"
              style={{
                background: university.trim() ? `linear-gradient(135deg, ${GREEN}, #16a34a)` : 'rgba(255,255,255,0.10)',
                color: university.trim() ? '#000' : 'rgba(255,255,255,0.30)',
        ...
        {step === 2 && (
          <div>
            <div className="text-center mb-8">
              <p className="text-white font-black text-2xl mb-2">Escolha a matéria para o diagnóstico</p>
              <p className="text-slate-500 text-sm">A IA vai testar seu nível atual com 10 flashcards táticos</p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              {(Object.entries(SUBJECT_META) as [SubjectId, typeof SUBJECT_META[SubjectId]][]).map(([id, meta]) => (
                <button key={id}
                  onClick={() => { setSubject(id); setStep(3); setCardIndex(0); setShowAnswer(false); setResults([]); setHealth(100); }}
        ...
        {/* ══ STEP 3: Flashcard Test ══ */}
        {step === 3 && subject && currentCard && (
          <div>
            {/* Progress + health */}
            <div className="mb-5">
              ...
              <div className="flex-1 flex flex-col justify-center">
                ...
                {!showAnswer && (
                  <div className="pt-4 border-t border-white/5">
                    <p className="text-slate-600 text-xs italic">{currentCard.q !== currentCard.a ? '← Tente lembrar antes de revelar' : ''}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {!showAnswer ? (
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                ] as { label: string; rating: Rating; color: string; bg: string; border: string }[]).map(({ label, rating, color, bg, border }) => (
        ...
        {step === 4 && (
          <div className="relative rounded-3xl p-7 sm:p-10" style={cardStyle}>
            <div className="absolute inset-x-0 top-0 h-px rounded-t-3xl"
              style={{ background: `linear-gradient(90deg, transparent, ${GREEN}80, ${VIOLET}50, transparent)` }} />

            {/* Score summary */}
            <div className="flex items-center gap-4 mb-7 p-4 rounded-2xl"
              style={{ background: health < 55 ? 'rgba(239,68,68,0.08)' : health < 72 ? 'rgba(249,115,22,0.08)' : 'rgba(34,197,94,0.08)',
                border: `1px solid ${health < 55 ? 'rgba(239,68,68,0.25)' : health < 72 ? 'rgba(249,115,22,0.25)' : 'rgba(34,197,94,0.25)'}` }}>
              <div className="text-4xl">
                {health < 55 ? '🚨' : health < 72 ? '⚠️' : '🎯'}
              </div>
              <div>
                <p className="text-white font-black text-base">
                  Status: <span style={{ color: health < 55 ? RED : health < 72 ? '#f97316' : GREEN }}>
                    {health < 55 ? 'CRÍTICO' : health < 72 ? 'EM RISCO' : 'CONTROLADO'}
                  </span>
                </p>
                <p className="text-slate-400 text-sm mt-0.5">
                  Saúde da Memória: <strong className="text-white">{health}%</strong> ·{' '}
                  {results.filter(r => r.rating === 'dificil').length} lacunas críticas detectadas
                </p>
              </div>
            </div>

            <p className="text-white font-black text-xl mb-1">Gerar meu Relatório de Lacunas</p>
            <p className="text-slate-500 text-sm mb-6">
              A IA vai analisar seus resultados e enviar um diagnóstico completo com o plano de ataque.
            </p>

            <form onSubmit={handleSubmitLead} noValidate className="flex flex-col gap-3">
              {[
                ...
                <div key={label}>
                  ...
                  <input type={type} placeholder={ph} value={val} onChange={e => set(e.target.value)}
                    className="w-full rounded-xl px-4 py-3 text-sm text-white outline-none placeholder-slate-700"
              ...
              <button type="submit" disabled={saving}
                ...
                {saving ? 'Gerando Relatório...' : '🧠 Ver meu Diagnóstico Completo →'}
              </button>

              <p className="text-center text-slate-700 text-xs mt-1">🔒 Seus dados são privados e protegidos.</p>
            </form>
          </div>
        )}
