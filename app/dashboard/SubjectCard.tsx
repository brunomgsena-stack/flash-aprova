...
export default function SubjectCard({ id, title, icon, color, domain, onLockedClick }: Props) {
  ...
  const inner = (
    <>
      ...
      <h2 className="text-xl font-bold text-white mb-1 relative z-10">{title}</h2>
      {domain && domain.level > 0 ? (
        <div className="flex items-center justify-between mt-3 relative z-10">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-medium" style={{ color: domain.color }}>{domain.label}</span>
          </div>
          <span className="text-xs text-slate-600">{Math.round(domain.coverage * 100)}%</span>
        </div>
      ) : (
        <p className="text-xs text-slate-600 mt-3 relative z-10">Nenhum card estudado</p>
      )}
      <p className="text-sm font-medium relative z-10 mt-2" style={{ color }}>
